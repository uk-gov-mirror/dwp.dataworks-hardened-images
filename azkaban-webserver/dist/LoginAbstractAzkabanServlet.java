...
public abstract class LoginAbstractAzkabanServlet extends AbstractAzkabanServlet {
  ...
  protected void doGet(final HttpServletRequest req, final HttpServletResponse resp)
    ...
    if (hasParam(req, "action") && getParam(req, "action").equals("login")) {
  ...
  private Session createSession(final HttpServletRequest req)
    ...
    if (hasParam(req, "cognitoToken")) {
      final String cognitoToken = getParam(req, "cognitoToken");
      return createSession(cognitoToken, ip);
    } else {
  ...
  private Session createSession(final String cognitoToken, final String ip)
      throws UserManagerException {
    final UserManager manager = getApplication().getUserManager();
    final User user = manager.getUser("cognitoToken", cognitoToken);
  ...
  protected void handleAjaxLoginAction(final HttpServletRequest req,
      ...
      throws ServletException {
    if ((hasParam(req, "username") && hasParam(req, "password")) || (hasParam(req, "cognitoToken"))) {
