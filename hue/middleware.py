...
from django.contrib.auth import REDIRECT_FIELD_NAME, BACKEND_SESSION_KEY, authenticate, load_backend, login, logout
...
class EnvironmentMiddleware(object):
    ...
    def __init__(self):
      if not 'desktop.auth.backend.RemoteUserDjangoBackend' in AUTH.BACKEND.get():
        LOG.info('Unloading EnvironmentMiddleware')
        raise exceptions.MiddlewareNotUsed
    ...
    def _remove_invalid_user(self, request):
        ...
        try:
            ...
            if isinstance(stored_backend, django.contrib.auth.backends.RemoteUserBackend):
                logout(request)
